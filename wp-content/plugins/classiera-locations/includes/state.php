...
function state_meta_box_cb( $post )
{
    ...
    <p>
        ...
        <select name="state_meta_box_country" id="state_meta_box_country" class="required" title="Please Select Country" required>
            <?php foreach($country_array as $key=>$val){?>
            <option value="<?php echo $key;?>" <?php selected( $selected[0], $key ); ?>><?php echo $val;?></option>
            <?php }?>
        </select>
		<div class="state-meta">
			<p><?php esc_html_e('Please enter the states followed by commas (example : Bedfordshire,Bristol,Cambridgeshire). Please do not include empty spaces or line breaks.', 'classiera-locations') ?></p>
