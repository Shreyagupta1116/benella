...
function city_meta_box_cb( $post )
{
    ...
    ?>
    <p>
        <label for="city_meta_box_country">
			<strong><?php esc_html_e('Country', 'classiera-locations') ?>:</strong> 
		</label>
    </p>
       <select name="city_meta_box_country" id="city_meta_box_country" class="required" title="Please Select Country" required>
	...
	<p>
        ...
        <select name="city_meta_box_state" id="city_meta_box_state" class="required" title="Please Select State" required>
		<option value="<?php echo $currentState;?>" selected><?php echo $currentState;?></option>
		</select>
    <div class="state-city">
		<p><?php esc_html_e('Please enter the cities followed by commas (example : London,Purley,Redhill). Please don&rsquo;t include empty spaces or line breaks.', 'classiera-locations') ?></p>
