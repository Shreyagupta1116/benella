...
class TWRecentPostWidget extends WP_Widget{
    public function __construct() {		
		...
		parent::__construct( 'TWRecentPostWidget', esc_html__( 'Classiera recent posts', 'classiera-helper' ), $widget_ops );
    }
			...
			<div class="media footer-pr-widget-v1">
				<div class="media-left">
					<a class="media-img" href="<?php echo esc_url(get_permalink($post->ID)); ?>">
						...
						?>
						<img class="media-object" src="<?php echo esc_url($classieraThumSrc[0]); ?>" alt="<?php echo get_the_title();?>">
						<?php }else{
							$classieraDEFThumb = get_template_directory_uri() . '/images/nothumb.png'
						?>
						<img class="media-object" src="<?php echo esc_url($classieraDEFThumb); ?>" alt="<?php echo get_the_title();?>">	
							<?php
						}?>
					</a>
				</div>
				<div class="media-body">
					...
					?>
					<span class="category">
						<?php esc_html_e( 'Category', 'classiera-helper'); ?>: 
			   ...
			   ?>
				...
				<li>
					<span>
						...
						$classieraALT = get_post_meta($thumb_id, '_wp_attachment_image_alt', true);
						?>
						<img class="img-rounded" src="<?php echo esc_url($classieraThumSrc[0]); ?>" alt="<?php echo get_the_title();?>">
						<?php }else{
							$classieraDEFThumb = get_template_directory_uri() . '/images/nothumb.png'
						?>
						<img class="img-rounded" src="<?php echo esc_url($classieraDEFThumb); ?>" alt="<?php echo get_the_title();?>">	
							<?php
						}?>
						<?php
						$post_currency_tag = get_post_meta($post->ID, 'post_currency_tag', true);
			...
			<div class="media footer-pr-widget-<?php echo esc_attr($widget_style); ?>">
				...
				<div class="media-body">
					...
					<?php }elseif($widget_style == 'v2'){?>
						...
						<?php $classieraPostPrice = get_post_meta($post->ID, 'post_price', true); ?>
						<?php if($classieraPostPrice){ ?>
						<p class="price"><?php esc_html_e( 'Price', 'classiera-helper'); ?> : <span class="color">
							<?php 
							if(is_numeric($classieraPostPrice)){
								echo classiera_post_price_display($post_currency_tag, $classieraPostPrice);
							}else{ 
    ...
    function form($instance) {
        ...
        <p>
            <label for="<?php echo esc_attr($this->get_field_id('title')); ?>">
				<?php esc_html_e('Title:', 'classiera-helper');?>
			</label>
        ...
        <p>
            <label for="<?php echo esc_attr($this->get_field_id('post_order')); ?>"><?php esc_html_e('Post order', 'classiera-helper');?>:</label>
            <select class="widefat" id="<?php echo esc_attr($this->get_field_id('post_order')); ?>" name="<?php echo esc_attr($this->get_field_name('post_order')); ?>">
                <option value="latest" <?php if ($post_order == 'latest') print 'selected="selected"'; ?>><?php esc_html_e('Latest Ads', 'classiera-helper');?></option>
                <option value="commented" <?php if ($post_order == 'commented') print 'selected="selected"'; ?>><?php esc_html_e('Featured Ads', 'classiera-helper');?></option>
            </select>
        </p>
		<p>
            <label for="<?php echo esc_attr($this->get_field_id('widget_style')); ?>"><?php esc_html_e('Widget Style', 'classiera-helper');?>:</label>
            <select class="widefat" id="<?php echo esc_attr($this->get_field_id('widget_style')); ?>" name="<?php echo esc_attr($this->get_field_name('widget_style')); ?>">
                <option value="v1" <?php if ($widget_style == 'v1') print 'selected="selected"'; ?>><?php esc_html_e('Style 1', 'classiera-helper');?></option>
                <option value="v2" <?php if ($widget_style == 'v2') print 'selected="selected"'; ?>><?php esc_html_e('Style 2', 'classiera-helper');?></option>
            </select>
        </p>
       <?php 
        ...
        if (count($customTypes) > 0) { ?>
            <p style="display: none;">
                <label for="<?php echo esc_attr($this->get_field_id('post_type')); ?>"><?php esc_html_e('Post from', 'classiera-helper');?>:</label>
        ...
        <p><?php esc_html_e('If no categories are selected, all categories ads will be shown', 'classiera-helper');?></p>
        ...
        <p>
            <label for="<?php echo esc_attr($this->get_field_id('number_posts')); ?>"><?php esc_html_e('Number of posts to show', 'classiera-helper');?>:</label>
...
function register_TWRecentPostWidget() {
    register_widget( 'TWRecentPostWidget' );
}
add_action( 'widgets_init', 'register_TWRecentPostWidget' );
add_action('wp_ajax_classiera_recent_post_terms', 'get_post_type_terms');
